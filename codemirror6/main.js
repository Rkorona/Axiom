...
/**
 * 计算剪切 / 复制应作用的文本范围。
 * 有选区时直接使用选区；无选区时退化为光标所在整行（含换行符，若非最后一行），
 * 这是主流编辑器（VSCode、Android Studio 等）在无选区时执行剪切/复制的通用行为。
 */
function getCutCopyRange() {
...
window.editorAPI = {
  ...
  redo: () => {
    const applied = redo(view)
    view.focus()
    return applied
  },

  /** 是否存在可撤销的历史记录，供 Android 端控制按钮可用状态 */
  canUndo: () => undoDepth(view.state) > 0,

  /** 是否存在可重做的历史记录，供 Android 端控制按钮可用状态 */
  ...
  // ── 剪贴板操作 ──────────────────────────────────────────────
  // 若当前没有选区，比照主流编辑器习惯（VSCode/Android Studio 等），
  // 剪切/复制退化为对光标所在整行生效，而不是静默地无操作。

  copySelected: () => {
    const { from, to } = getCutCopyRange()
    return b64Encode(view.state.doc.sliceString(from, to))
  },
  ...
  /** 粘贴文本（Base64 编码）。语义等价于 insertTextBase64，命名更贴合"粘贴"场景 */
  paste: (b64) => window.editorAPI.insertTextBase64(b64),

package com.server.auditor.ssh.client.contracts.team;

import java.util.Iterator;
import moxy.viewstate.MvpViewState;
import moxy.viewstate.ViewCommand;
import moxy.viewstate.strategy.AddToEndSingleStrategy;
import moxy.viewstate.strategy.OneExecutionStateStrategy;

/* JADX INFO: loaded from: classes2.dex */
public class c extends MvpViewState<d> implements d {

    public class a extends ViewCommand<d> {
        a() {
            super("initView", AddToEndSingleStrategy.class);
        }

        @Override // moxy.viewstate.ViewCommand
        /* JADX INFO: renamed from: a, reason: merged with bridge method [inline-methods] */
        public void apply(d dVar) {
            dVar.a();
        }
    }

    public class b extends ViewCommand<d> {

        /* JADX INFO: renamed from: a, reason: collision with root package name */
        public final int f23129a;

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        public final String f23130b;

        b(int i10, String str) {
            super("navigateToSetupTeamVaultCredentialsModeScreen", OneExecutionStateStrategy.class);
            this.f23129a = i10;
            this.f23130b = str;
        }

        @Override // moxy.viewstate.ViewCommand
        /* JADX INFO: renamed from: a, reason: merged with bridge method [inline-methods] */
        public void apply(d dVar) {
            dVar.t6(this.f23129a, this.f23130b);
        }
    }

    /* JADX INFO: renamed from: com.server.auditor.ssh.client.contracts.team.c$c, reason: collision with other inner class name */
    public class C0360c extends ViewCommand<d> {
        C0360c() {
            super("navigateToTeamPresenceScreen", OneExecutionStateStrategy.class);
        }

        @Override // moxy.viewstate.ViewCommand
        /* JADX INFO: renamed from: a, reason: merged with bridge method [inline-methods] */
        public void apply(d dVar) {
            dVar.D0();
        }
    }

    /* JADX WARN: Type inference incomplete: some casts might be missing */
    @Override // com.server.auditor.ssh.client.contracts.team.d
    public void D0() {
        C0360c c0360c = new C0360c();
        this.viewCommands.beforeApply(c0360c);
        if (hasNotView().booleanValue()) {
            return;
        }
        Iterator it = this.views.iterator();
        while (it.hasNext()) {
            ((d) it.next()).D0();
        }
        this.viewCommands.afterApply(c0360c);
    }

    /* JADX WARN: Type inference incomplete: some casts might be missing */
    @Override // com.server.auditor.ssh.client.contracts.team.d
    public void a() {
        a aVar = new a();
        this.viewCommands.beforeApply(aVar);
        if (hasNotView().booleanValue()) {
            return;
        }
        Iterator it = this.views.iterator();
        while (it.hasNext()) {
            ((d) it.next()).a();
        }
        this.viewCommands.afterApply(aVar);
    }

    /* JADX WARN: Type inference incomplete: some casts might be missing */
    @Override // com.server.auditor.ssh.client.contracts.team.d
    public void t6(int i10, String str) {
        b bVar = new b(i10, str);
        this.viewCommands.beforeApply(bVar);
        if (hasNotView().booleanValue()) {
            return;
        }
        Iterator it = this.views.iterator();
        while (it.hasNext()) {
            ((d) it.next()).t6(i10, str);
        }
        this.viewCommands.afterApply(bVar);
    }
}

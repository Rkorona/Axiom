 ...
 *
 * Extends [SchemeDarcula] so that every unoverridden colour (scrollbars, edge
 * effects, auto-complete popup, selection handles, etc.) starts from a dark
 * baseline instead of the light default.  Only Axiom-specific accents are
 * overridden on top.
 *
 * Colour decisions:
 *  - Background / gutter  → AxiomInk / flush (near-black deep-space)
 ...
 *  - Strings              → set via TextMate scope selectors once grammars are loaded
 */
class AxiomEditorColorScheme : SchemeDarcula() {

    override fun applyDefault() {
        super.applyDefault()   // dark baseline: scrollbars, popups, edge effects all dark
        ...
        // ── Syntax tokens (active when a language is attached) ─────────────────
